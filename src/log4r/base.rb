...
module Log4r
  ...
  def Log4r.const_get_ary(ary, const=Log4r)
    ary.inject(const){|s,e|s and s.const_get(e) rescue nil}
  end
  def Log4r.find_const(str)
    ns=str.split('::')
    ret = Log4r.const_get_ary(ns) || Log4r.const_get_ary(ns,Kernel)
    return ret if ret
    [ ns.map(&:downcase).join('/') , ns.last.downcase, "log4r/outputter/#{ns.last.downcase}"].each do |possible_file|
      begin
    ...
    ret = Log4r.const_get_ary(ns) || Log4r.const_get_ary(ns,Kernel)
    raise "Unknown constant, #{str}" unless ret
    ret
  end
