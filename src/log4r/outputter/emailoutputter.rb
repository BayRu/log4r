...
module Log4r
  ...
  class EmailOutputter < Outputter
    ...
    def validate(hash)
      @buffsize = (hash[:buffsize] or hash['buffsize'] or hash['msgs_per_email'] or hash[:msgs_per_email] or 100).to_i
    ...
    def decode_immediate_at(hash)
      @immediate = Hash.new
      _at = (hash[:immediate_at] or hash['immediate_at'] or hash['no_buffering'] or hash[:no_buffering])
      return if _at.nil?
    ...
    def canonical_log(event)
      synch {
        @buff.push @formatfirst ? format(event) : event
        send_mail if @buff.size >= @buffsize or @immediate[event.level]
      }
    end

    def send_mail
      msg = @formatfirst ? @buff.join : @buff.collect{|e| format(e)}.join
