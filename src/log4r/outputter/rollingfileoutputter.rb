...
module Log4r
  ...
  class RollingFileOutputter < FileOutputter
    ...
    def initialize(_name, hash=Log4rHash.new)
      @count = 0
      super( _name, hash.merge({:create => false,:no_file_mangling => true}) )
	...
	# try to disable out a file that is not actually opened because
	...
	if ( @baseFilename != @filename ) then
	  close
        ...
        Logger.log_internal {
          "RollingFileOutputter '#{@name}' could not disable #{@filename}"
        }
      end
      @count += 1
      close
    ...
    def reopen
	    close
      makeNewFilename
      File.new(@filename, file_access_bits)
    end 
